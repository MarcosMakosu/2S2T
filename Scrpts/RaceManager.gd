...
func _ready() -> void:
	...
	# Busca o RaceManager pelo grupo
...
func _update_movement(delta: float) -> void:
	...
	var target_speed := difficulty.max_speed
...
func _apply_rubber_band(base_speed: float) -> float:
	...
	var player_pos := race_manager.get_player_race_position()
	var bot_pos := race_manager.get_bot_race_position(self)
	var diff := player_pos - bot_pos
	
	var factor := 1.0 - (diff * difficulty.rubber_band_strength)
...
func _request_next_checkpoint() -> void:
	if race_manager:
		var next := race_manager.get_next_checkpoint(self)
		if next:
